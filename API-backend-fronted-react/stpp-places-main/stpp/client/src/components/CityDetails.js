...
const CityDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5181/api/countries/${countryId}/cities/${cityId}`);
  ...
  const confirmed = window.confirm('Are you sure you want to delete this city?');
      ...
      await axios.delete(`http://localhost:5181p/api/countries/${countryId}/cities/${cityId}`, {
        headers: {
