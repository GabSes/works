...
const PlaceDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5181/api/countries/${countryId}/cities/${cityId}/places/${placeId}`);
    ...
    let accessToken = localStorage.getItem('accessToken');
            ...
            if (isAccessTokenExpired) {
              ...
              const response = await axios.post(
                'http://localhost:5181/api/accessToken',
                {
      ...
      await axios.delete(`http://localhost:5181/api/countries/${countryId}/cities/${cityId}/places/${placeId}`, {
                                                                                                                                          headers: {
