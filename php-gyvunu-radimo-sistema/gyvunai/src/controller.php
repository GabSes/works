   ...
   public function handleCreateNewAdButton()
{
    if (isset($_POST['createad_btn'])) {
        ...
        if (empty($_POST['title']) || empty($_POST['text']) || empty($_POST['age']) || empty($_POST['valid_till']) || empty($_POST['type']) || empty($_POST['species']) || empty($_POST['gender']) || empty($_POST['place']|| empty($_POST['phone']))){
 ...
 if (!empty($_FILES['image']['tmp_name'])) {
            ...
            // If no image is uploaded, set $imageData to null or an appropriate default
            $imageData = null;
        }

        if ($canICreateNewAd) {
            if ($this->model->createNewAd(
                $_POST['title'],
                $_POST['text'],
                $_POST['age'],
                $_POST['valid_till'],
...
{
    $speciesFilter = isset($_GET['species']) ? $_GET['species'] : null;

    $searchJobArr = $this->model->getSearchJobListGlobal($speciesFilter);
    $giveJobArr = $this->model->getGivingJobListGlobal($speciesFilter);

    // Print the ads
    $this->view->printGlobalAdsContent($searchJobArr, $giveJobArr);

    // Check if the user has the privilege to delete ads
    if ($_SESSION['role'] == 2) {
        // Print the form to select ads for deletion
        $adsForDeletion = $this->model->getAdsForDeletion();
        $this->view->printAdsDeletionForm($adsForDeletion);
    }
}
	...
	public function handleAdsDeletion()
{
    if (isset($_POST['deleteSelectedAds']) && isset($_POST['adsToDelete'])) {
        $adsToDelete = $_POST['adsToDelete'];

        foreach ($adsToDelete as $adId) {
            // You might want to add additional validation here to ensure $adId is valid
            $this->model->hideAd($adId);
        }

        $this->view->printSuccess("Selected ads deleted successfully!");
        $this->redirect_to_another_page("ads.php", 1);
    }
}
